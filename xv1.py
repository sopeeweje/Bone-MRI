...
def train(model, training, validation, run_id):
    ...
    history = model.fit_generator(
        training,
        # should these be changed for x validation?
...
def run(loaded_data, run_id=None, input_form=config.INPUT_FORM, hyperparameters=dict()):
    if run_id is None:
        run_id = int(datetime.utcnow().timestamp())

    # not really going to use the holdout_test here
    training, validation, test, holdout_test = loaded_data
    # new model instance - SHOULD THIS BE THE TRAINING SIZE OR TRAINING + VALIDATION SIZE?
